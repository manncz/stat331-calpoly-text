...
::: learn-more
### Learn more about tibbles {.unnumbered}
...
In this section, we're going start learning how to work with data. Generally speaking, data doesn't come in a form suitable for data visualization or statistical analysis[^04-data-transformations-1] - you have to clean it up, create the variables you care about, get rid of those you don't care about, and so on.

[^04-data-transformations-1]: See [this twitter thread](https://twitter.com/JennyBryan/status/722954354198597632) for some horror stories. [This tweet](https://twitter.com/jengolbeck/status/1153064308483510272?s=20) is also pretty good at showing one type of messiness.
...
We will be using the `tidyverse` for this. It's a meta-package (a package that just loads other packages) that collects packages designed with the same philosophy[^04-data-transformations-2] and interface (basically, the commands will use predictable argument names and structure). You've already been introduced to parts of the tidyverse - specifically, `readr` and `ggplot2`.

[^04-data-transformations-2]: The philosophy includes a preference for pipes, but this preference stems from the belief that code should be readable in the same way that text is readable.
...
## Motivation & Example Dataset {.unnumbered}
...
::: note
#### Useful comparison operations in R {.unnumbered}
...
#### Filtering by row number {.unnumbered}
...
#### `arrange()` Sorting rows by variable values {.unnumbered}
...
First, dplyr aims to work with standard R syntax, making it intuitive (and also, making it work with variable names instead of just variable indices).[^04-data-transformations-3]\
...
[^04-data-transformations-3]: It accomplishes this through the magic of quasiquotation, which we will not cover in this course because it's basically witchcraft.
