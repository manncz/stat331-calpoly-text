...
#R^2 is the variance of the predictions divided by the variance of the outcome
var(test_preds)/var(test_dat$bill_length_mm)
```

Thus our test or validation $R^2$ is `r round(var(test_preds)/var(test_dat$bill_length_mm), 4)`. The $R^2$ for the training data was 0.0391. Since the testing and training metrics are pretty similar, there isn't evidence of overfitting.
...
for(x in 1:k){
  ...
  fold_resid <- fold_dat$bill_length_mm - fold_preds

  # calculate R2 for the held-out fold data and save it
  cv_r2[x] <- 1 - var(fold_resid)/var(fold_dat$bill_length_mm)
